...
export type InferOk<TResult> =
  TResult extends ResultAsync<infer TOk, unknown>
    ? TOk
    : TResult extends Result<infer TOk, unknown>
      ? TOk
      : never

export type InferErr<TResult> =
  TResult extends ResultAsync<unknown, infer TErr>
    ? TErr
    : TResult extends Result<unknown, infer TErr>
      ? TErr
      : never
...
export const combineResultList = <T, E>(
  ...
  for (const result of resultList) {
    ...
    acc.map((list) => void list.push(result.value))
  }
  return acc
}
